...
namespace TestHost
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            Assembly testAssembly = Assembly.GetAssembly(typeof(TestsFromDNAAlignments));
                ...
                if (found)
                {
                    ...
                    foreach (MethodInfo method in methods)
                    {
                        ...
                        foreach (Attribute attr in method.GetCustomAttributes())
                        {
                            ...
                            if (attr is ProgressTestAttribute)
                            {
                                hasProgress = true;
                            }
                                        ...
                                        new Progress<double>(x =>
                                        {
                                            lock (progressLock)
                                            {
                                                if (!done)
                                                {
                                                    ...
                                                    string prog = x.ToString("0.00%", System.Globalization.CultureInfo.InvariantCulture);
                                                    Console.Write(prog + new string(' ', 7 - prog.Length));
                                                }
                                            }
                                        })
                                    });
